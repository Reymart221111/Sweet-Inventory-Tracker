<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-show="openEditModal"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl mx-4" x-show="openEditModal"
        ...
        <form wire:submit.prevent="updateProduct" class="p-6 space-y-4" x-data="{
            open: false,
            ...
            resetImage() {
                this.image = null;
                this.imagePreview = null;
                const fileInput = this.$el.querySelector('#image');
                if (fileInput) {
                    fileInput.value = '';
                }
            }
        }" @update-event.window="resetImage">
            ...
            <div class="mt-4">
                ...
                <div class="flex items-center justify-center w-full">
                    <label
                        ...
                        <input type="file" name="image" id="image" wire:model.live='image' accept="image/*"
                            @change="handleImageUpload($event)" class="opacity-0 absolute inset-0 cursor-pointer" />
                    </label>
                </div>
